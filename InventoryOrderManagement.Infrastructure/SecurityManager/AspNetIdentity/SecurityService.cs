...
using Constants = InventoryOrderManagement.Core.Common.Constants;
...
public static class RoleHelper
{
    ...
    public static string GetProfileRole(bool isAdmin)
    {
...
public class SecurityService
{
        ...
        )
    {
        ...
        token.ExpiryDate = DateTime.UtcNow.AddDays(Constants.TokenConsts.ExpiryInDays);
        ...
        return new LoginResultDto
        {
            ...
            Roles = roles.ToList(),
            Avatar = user.ProfilePictureName ?? string.Empty,
            Token = accessToken,
            Message = "Login başarılı",
            UserName = user.UserName ?? string.Empty,
            TokenExpireDate = DateTime.UtcNow.AddMinutes(60),
            RefreshTokenExpireDate = DateTime.UtcNow.AddDays(Constants.TokenConsts.ExpiryInDays),
            Success = true
        };
    }
        ...
        )
    {
        ...
        user.EmailConfirmed = _identitySettings?.SignIn?.RequireConfirmedEmail == true ? false : true;
        ...
        if (!await _userManager.IsInRoleAsync(user, RoleHelper.GetProfileRole(false)))
        {
            await _userManager.AddToRoleAsync(user, RoleHelper.GetProfileRole(false));
        }

        if (_identitySettings?.SignIn?.RequireConfirmedEmail == true)
        {
